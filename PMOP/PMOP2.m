function varargout = PMOP2(Operation,Global,input)
% <problem> <PMOP>
        ...
        case 'value'
           ...
           %% construct g function:
            ...
            if  Linkage ~= 0   %% l2 linkage function in g1
                temp2 = zeros(N,D-M+1);
                for i=1:D-M+1
                   temp2(:,i) =  (1+ cos(0.5.*pi.*i./(D-M+1))).*(PopDec(:,M-1+i) - Global.lower(M-1+i)) - PopDec(:,1).*(Global.upper(M-1+i) - Global.lower(M-1+i));   
                end  
                temp2 = temp2.^2;
                g = sum(temp2,2); %% g2...       
            end
