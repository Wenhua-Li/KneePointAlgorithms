function varargout = DEB3DK(Operation,Global,input)
% <problem> <KneeProblem>
% Comparison of Multiobjective Evolutionary Algorithms for knee
% identification
% operator --- EAreal
    K = 2; %% control the number of knees.
    switch Operation
        case 'init'
            Global.M        = 3;
            Global.D        = 12;
...
%             PopObj(:,3) = g.*r.*cos(PopDec(:,1).*pi./2);
            ...
            k = zeros(N,1);
            for i=1:M-1
             r(:,i) = 5+10.*(PopDec(:,i)-0.5).*(PopDec(:,i)-0.5)+2.*cos(2.*K.*pi.*PopDec(:,i))./K;
            end
            k = sum(r,2)./(M-1); %% k1...
            
%             k=k.*(1+9.*sum(PopDec(:,2:end),2)./(Global.D -1));

            PopObj(:,1) = k.*sin(PopDec(:,1).*pi./2).*sin(PopDec(:,2).*pi./2);
            PopObj(:,2) = k.*sin(PopDec(:,1).*pi./2).*cos(PopDec(:,2).*pi./2);
            PopObj(:,3) = k.*cos(PopDec(:,1).*pi./2);
